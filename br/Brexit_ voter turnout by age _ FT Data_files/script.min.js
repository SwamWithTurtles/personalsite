...
return{supportsCrossDomain:supportsCrossDomain,getNavigationHTML:getNavigationHTML,getHeadlinesJSON:getHeadlinesJSON};}());var FT=FT||{};FT.navigation=(function(){var navEl,navWidth,firstLevelEls,headlinesTTL=300000,maxHeadlines=10,flyoutInView,timers={},timeOut,timeOutThirdMenu;function init(){navEl=FT.$("#nav-ftcom");firstLevelEls=navEl.find(".nav-item-l1");navWidth=navEl.width();function flyoutsLoaded(){prepareForHeadlines();listenForUserInput();}
...
var li=FT.$(this);li.find("a").trigger("click");li.find("ol").remove();});navEl.on("mouseenter.nav",".nav-item-l1 > a",function(ev){timeOut=setTimeout(function(){if(typeof(timeOut)!=='undefined'){showSecondLevel(FT.$(ev.target).closest(".nav-item-l1"));}},200);});navEl.on("mouseleave.nav",".nav-item-l1",function(ev){clearTimeout(timeOut);hideSecondLevel(FT.$(ev.target).closest(".nav-item-l1"));});navEl.on("mouseenter.nav",".nav-has-l3 > a",function(ev){timeOutThirdMenu=setTimeout(function(){if(typeof(timeOutThirdMenu)!=='undefined'){showThirdLevel(FT.$(ev.target));}},200);});navEl.on("mouseleave.nav",".nav-has-l3",function(ev){clearTimeout(timeOutThirdMenu);hideThirdLevel();});FT.$(document).on("keydown.nav",".nav-item-l1.nav-has-flyout > a",function(ev){var firstLevelEl=FT.$(ev.target).closest(".nav-item-l1");if(ev.keyCode===40){showSecondLevel(firstLevelEl);}else if(ev.keyCode===38){hideSecondLevel(firstLevelEl);}});}
function hideSecondLevel(firstLevelEl){FT.$('.nav-has-flyout').removeClass("nav-show-flyout");FT.$('.nav-flyout').css('display','none');flyoutInView=null;}
...
hideSecondLevel(flyoutInView);flyoutInView=firstLevelEl;checkHeadlines(firstLevelEl);positionFlyout(firstLevelEl);firstLevelEl.addClass("nav-show-flyout");firstLevelEl.children('.nav-flyout').css('display','block');}
function hideThirdLevel(){FT.$(".nav-items-l3").hide();}
function showThirdLevel(secondLevelEl){hideThirdLevel();secondLevelEl.siblings(".nav-items-l3").show();}
